...
Java 26 contains eight C1 executable examples and two C2 explanatory modules. HTTP/3 is a C1 executable example because the JDK 26 API surface is final and can be demonstrated deterministically without live networking. Final-field restrictions are C1 because the Java 26 warning can be captured in an isolated child JVM. Applet API removal is also C1 because the removed package can be verified with a deterministic child-compiler example. AOT object caching is C1 because cache creation and reuse can be verified in isolated child JVMs without measuring startup performance. PEM encodings are C1 because a deterministic in-memory envelope round trip can be compiled and run in an isolated JDK 26 child JVM with matching preview flags. Lazy Constants are C1 because deferred initialization, one-time evaluation, and value reuse can be verified with an in-memory supplier in an isolated JDK 26 child JVM. Primitive patterns are C1 because their preview syntax can be compiled and run in an isolated JDK 26 child JVM with matching preview flags. Structured Concurrency is C1 because scope joining and failure propagation can be verified in an isolated JDK 26 child JVM without relying on scheduler timing or cancellation races. G1 synchronization reduction and the Vector API remain C2 explanatory modules because their runtime and incubator behavior is not represented by a focused portable example.
