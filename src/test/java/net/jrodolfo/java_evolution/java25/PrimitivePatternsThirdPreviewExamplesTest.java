...
class PrimitivePatternsThirdPreviewExamplesTest {

	private final PrimitivePatternsThirdPreviewExamples examples = new PrimitivePatternsThirdPreviewExamples();
	...
	void childProcessDemonstratesPrimitivePatternMatching(@TempDir Path workspace) throws Exception {
		assumeTrue(Runtime.version().feature() == 25,
				"Java 25 preview syntax must be compiled with a JDK 25 preview compiler");

		PrimitivePatternsThirdPreviewExamples.PrimitivePatternsWorkflowResult result =
				examples.runPrimitivePatternsWorkflow(workspace);

		assertThat(result.compilation().exitCode())
				.as("the generated source should compile when Java 25 preview syntax is enabled")
				.isZero();
	...
	@Test
	void probeSourceUsesRealJava25PrimitivePatternSyntax() {
		assertThat(examples.probeSource())
				.as("the child source should demonstrate the Java 25 preview primitive-pattern syntax directly")
