...
When demonstrating a preview feature from Java version `n`, an executable example may use later final syntax only when that syntax is also syntactically and semantically faithful to the Java `n` preview. The README for the example must identify the feature as preview in Java `n`. When the later syntax differs materially, the repository uses a historically faithful source snippet, generated source, descriptive model, or another suitable approach instead. Building with JDK 26 alone is not a reason to use later syntax in an earlier preview example.
